...
describe('basic test endpoints', () => {
  ...
  describe('auth endpoints', () => {
    test('whoami', async () => {
      const server = new ServerQuery(registry.getRegistryUrl());
      (await server.whoami()).status(HTTP_STATUS.OK);
      const q1 = await server.logout(registry.getToken());
      q1.status(HTTP_STATUS.FORBIDDEN);
    });
  });
  ...
  describe('publish package', () => {
    test('should create a package', async function () {
      const server = new ServerQuery(registry.getRegistryUrl());
      await await server.addPackage('testpkg-single-tarball-1');
    });
  ...
  describe('get packages', () => {
    test('should get a private package', async function () {
      ...
      await (await server.getPackage('new-package')).status(200);
      await (
        await server.getTarball('new-package', 'new-package-1.0.0.tgz')
    ...
    test('should get error on wrong a private package', async function () {
      ...
      await (await server.getPackage('non-exist-package')).status(HTTP_STATUS.OK);
      await (
        await server.getTarball('non-exist-package', 'non-exist-package-1.0.1.tgz')
      ).status(404);
    });
  });
  ...
  describe('unpublish package', () => {
    test('shoud unpublish the whole package of many published', async function () {
      ...
      await (await server.getPackage('unpublish-new-package')).status(HTTP_STATUS.OK);
      await (
        await server.removePackage('unpublish-new-package', '_rev')
      ).status(HTTP_STATUS.CREATED);
      await (await server.getPackage('unpublish-new-package')).status(HTTP_STATUS.NOT_FOUND);
    });
  });

  // test('downloading non-existent tarball', async () => {
  //   const server = new ServerQuery(registry.getRegistryUrl());
  //   (await server.getTarball('not-fund-tarball', TARBALL))
